...
<div class="portlet light ">
    <div class="portlet-title">
        <div class="caption font-green">
            ...
            <span class="caption-subject bold uppercase"> Ubah berita</span>
        </div>
        <div class="actions">
            <div class="btn-group">
                <a class="btn btn-sm btn-warning" href="{{url('/panel/berita')}}"> <i class="fa fa-arrow-left"></i> Kembali
                </a>
            </div>
        </div>
    </div>
    <div class="portlet-body form">
        <form role="form" method="POST" action="{{url('panel/berita/'.$post->id)}}">
        {{ csrf_field() }}{{ method_field('PUT') }}
            <div class="form-body">
                <div class="form-group form-md-line-input form-md-floating-label">
                    <input type="text" class="form-control" id="form_control_1" name="judul" value="{{$post->judul}}" >
                    <label for="form_control_1">Judul Berita</label>
                    <span class="help-block">Masukkan judul berita (singkat, padat, jelas)</span>
                </div>
                <div class="form-group form-md-line-input form-md-floating-label col-md-3">
                    <div class="row">
                        ...
                        @foreach($stats as $stat)
                            <option value="{{$stat->nama_status}}" {{ $stat->nama_status == $post->status ? 'selected="selected"' : '' }} >{{$stat->nama_status}}</option>
                        @endforeach
                        </select>
                ...
                <div class="form-group form-md-line-input form-md-floating-label col-md-3">
                        <div class="input-group input-medium date date-picker" data-date-format="yyyy-mm-dd">
                            <input type="text" class="form-control" id="form_control_1" name="tgl_buat" value="{{$post->tgl_buat}}" readonly>
                ...
                <div class="form-group form-md-line-input form-md-floating-label col-md-6">
                    <div class="input-group">
                        <input id="thumbnail" class="form-control" type="text" name="filepath" value="{{$post->gambar}}">
                ...
                <div class="form-group last">
                    ...
                    <textarea id="siduy" name="berita">{!! $post->berita !!}</textarea>
                </div>
            </div>
            <div class="form-actions noborder">
                <button type="submit" class="btn blue">Simpan</button>
                <a href="{{url('/panel/berita')}}" class="btn default">Batal</a>
            </div>
        </form>
    </div>
</div>
