...
<div class="portlet light ">
    ...
    <div class="portlet-body form">
        <form role="form" method="POST" action="{{url('panel/pengguna/'.$user->id)}}">
        {{ csrf_field() }}{{ method_field('PUT') }}
            <div class="form-body">
                <div class="form-group form-md-line-input form-md-floating-label">
                    <input type="text" class="form-control" id="form_control_1" name="name" value="{{ $user->name }}" >
                    <label for="form_control_1">Nama pengguna</label>
                    <span class="help-block">Masukkan nama pengguna...</span>
                </div>
                <div class="form-group form-md-line-input form-md-floating-label">
                    <input type="text" class="form-control" id="form_control_1" name="email" value="{{ $user->email }}" >
                ...
                <div class="form-group form-md-line-input form-md-floating-label">
                    ...
                    @foreach($levels as $level)
                        <option value="{{$level->id}}" {{ $level->id == $user->level_id ? 'selected="selected"' : '' }}>{{$level->nama_level}}</option>
                    @endforeach
                    </select>
                ...
                <div class="form-group form-md-line-input form-md-floating-label">
                    ...
                    <label for="form_control_1">Ganti Password</label>
